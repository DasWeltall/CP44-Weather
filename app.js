...
const ICONS = {
  play: `<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><polygon points="9 6 18 12 9 18" fill="currentColor"></polygon></svg>`,
  pause: `<svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><rect x="7.5" y="6" width="3.2" height="12" rx="1" fill="currentColor"></rect><rect x="13.3" y="6" width="3.2" height="12" rx="1" fill="currentColor"></rect></svg>`
};
...
function initDom() {
  ...
  dom.radarSlider = document.querySelector("#radar-slider");
  dom.radarPlay = document.querySelector("#radar-play");
}
...
function attachEvents() {
  ...
  dom.radarSlider?.addEventListener("input", handleRadarScrub);
  dom.radarSlider?.addEventListener("change", handleRadarScrub);
  dom.radarPlay?.addEventListener("click", toggleRadarPlayback);
...
function initRadar(location) {
  ...
  state.radar.map.createPane("radarPane");
  state.radar.map.getPane("radarPane").style.zIndex = 450;
  ...
  try {
    ...
    if (!frames.length) {
      ...
      state.radar.frameLayers.forEach((layer) => {
        if (layer && state.radar.map?.hasLayer(layer)) {
          state.radar.map.removeLayer(layer);
        }
      });
      state.radar.frameLayers = [];
      state.radar.position = 0;
      setRadarPlaying(false);
      updateRadarControlsState();
      return;
    }
...
function renderRadarFrame() {
  ...
  const maxIndex = state.radar.frames.length - 1;
  if (state.radar.position > maxIndex) {
    state.radar.position = maxIndex;
  }
  ...
  if (dom.radarSlider) {
    dom.radarSlider.value = state.radar.position.toString();
  }
}
...
function restartRadarAnimation() {
  stopRadarAnimation();
  if (!state.radar.frames.length || !state.radar.playing) return;
...
function stopRadarAnimation() {
  if (state.radar.animationTimer) {
    ...
    state.radar.animationTimer = null;
  }
}

function setRadarPlaying(playing) {
  state.radar.playing = Boolean(playing);
  if (!dom.radarPlay) return;
  dom.radarPlay.setAttribute("aria-pressed", state.radar.playing.toString());
  dom.radarPlay.setAttribute("aria-label", state.radar.playing ? "Radar pausieren" : "Radar abspielen");
  dom.radarPlay.title = state.radar.playing ? "Radar pausieren" : "Radar abspielen";
  dom.radarPlay.innerHTML = state.radar.playing ? ICONS.pause : ICONS.play;
}

function toggleRadarPlayback() {
  if (!state.radar.frames.length) return;
  const next = !state.radar.playing;
...
function handleRadarScrub(event) {
  if (!state.radar.frames.length) return;
  const value = Number(event.target.value);
  if (Number.isNaN(value)) return;
  state.radar.position = Math.min(Math.max(value, 0), state.radar.frames.length - 1);
  setRadarPlaying(false);
  stopRadarAnimation();
  renderRadarFrame();
  updateRadarControlsState();
...
function updateRadarControlsState() {
  const hasFrames = state.radar.frames.length > 0;
  const max = hasFrames ? state.radar.frames.length - 1 : 0;
  if (dom.radarSlider) {
    dom.radarSlider.disabled = !hasFrames;
    dom.radarSlider.max = max.toString();
    dom.radarSlider.value = Math.min(state.radar.position, max).toString();
  }
  if (dom.radarBack) {
    dom.radarBack.disabled = !hasFrames;
  }
  if (dom.radarForward) {
    dom.radarForward.disabled = !hasFrames;
  }
  if (dom.radarPlay) {
    dom.radarPlay.disabled = !hasFrames;
    if (!hasFrames) {
      dom.radarPlay.innerHTML = ICONS.play;
    }
  }
}
